...
Agent VM, o `avm`, es un control plane local para perfiles de agentes de IA de programación. Defines un agente una vez y renderizas ese perfil hacia runtimes como Codex, Claude Code, Cline y Cursor.

La apuesta: los desarrolladores no se van a estandarizar en un único coding agent. Falta un objeto portable que describa quién es el agente, qué puede usar, qué modelo prefiere, qué permisos tiene y qué memoria de largo plazo debe cargar.
