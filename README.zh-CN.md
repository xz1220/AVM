...
其他东西都是支撑机制。Skills 属于 Agent 配置，不应该成为用户主路径里的独立模块。已有本地配置导入只是创建 Agent 时的一种来源，不应该单独变成 “runtime discovery” 模块。同步 runtime 文件是 `avm use` 和受控激活背后的实现细节，不应该要求用户主动理解。
...
```text
Package / 已有 Agent / 本地 runtime 配置
  -> 创建 Agent
...
最后一项就是原来的“查看和导入已有本地配置”，但它不应该是独立模块，而应该融入 create 流程。
...
| 本地配置导入 | `runtime scan/list`、`--from-import` | 应融入 Agent 创建向导 |
