...
然后当前 Codex、Claude Code 或 Cline 就应该按这个 Agent Profile 工作。这个 Profile 应该包含角色、指令、模型、权限、能力引用和记忆引用，否则用户会分不清“到底该改 agent 还是改 env”。
...
用户在同一个 repo 里同时使用 Codex 写代码、Claude Code 做 review、Cline 处理 IDE 内上下文。用户执行 `avm use backend-dev` 后，Codex 默认使用 `backend-coder`，Claude Code 默认使用 `code-reviewer`，Cline 默认使用 `backend-assistant`。用户不需要在 env 里再次选择 Agent。
...
| 用户想做什么 | 推荐命令 | 用户心智 |
|--------------|----------|----------|
| 只想让 Codex/Claude/Cline 按某个角色工作 | `avm use backend-coder` | 激活一个 Agent Profile |
| 想在两个角色之间切换 | `avm use backend-coder` / `avm use tech-writer` | 切换当前 active profile |
| 想让 Codex 写代码、Claude Code review、Cline 辅助 IDE | `avm use backend-dev` | 激活一个多 runtime 工作场景 |
...
- 用户能创建至少 1 个 Environment，把 Codex/Claude Code/Cline 分别绑定到不同 Agent Profile。
...
- Codex/Cline 获得当前 Agent Profile 或 Environment 映射后的配置输出；Cursor 完成 rules/MCP 文件级 PoC。
