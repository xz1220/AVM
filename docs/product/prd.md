...
AVM 不应该让用户先理解 runtime 配置文件、skills registry、runtime scan、sync 或 memory 迁移。用户真正关心的是：
...
### 2.3 本地已有配置导入属于 create 来源

“查看和导入已有本地配置”不应该是一个独立模块。它的真实场景只有一个：

> 用户已经在 Codex、Claude Code、OpenCode 等工具里有配置，想把它抽成 AVM Agent。
...
底层可以继续使用 runtime scan/import report，但用户不需要把它理解成一个模块。
...
- Import local config 是 Agent create 的来源，不是主模块。
...
- 用户只理解“从我已有的 Claude Code/Codex/OpenCode 配置创建 Agent”。
- 不需要理解 `runtime scan` 或 `import-report.json`。
- AVM 只读扫描本地配置。
- 写入前展示将创建的 Agent。

### 6.4 创建多 runtime 工作环境
...
| Local import | runtime scan/list、from-import | 融入 agent create source picker |
...
- README 主路径不再要求用户理解 runtime scan、skill registry 或 sync。

P1 成功标准：

- 用户能完整 CRUD Environment。
- 用户能从已有 runtime 配置创建 Agent，但入口在 create 流程中。
