...
- 已有 `avm env create/list/show/edit/delete/rename/clone`。
- `env create` 和 `env edit` 支持交互式流程，也支持 flags 脚本化。
- `env show --resolved` 可查看继承 project override 后的实际生效结果。
- `env rename/delete` 会保护 active env 和当前项目 override 引用。
...
| Environment | `env create/list/show/edit/delete/rename/clone` | 更完整的交互体验、跨项目 override 引用发现 |
