...
> 最后更新：2026-04-24（v4 — Portable Memory 对齐）
...
5. 读写 registry capability 和 portable memory metadata。
...
├── project.go        # ProjectOverride 和项目级 agents
├── capability.go     # registry capability 读取
├── memory.go         # memory metadata 读取
├── resolve.go        # ResolvedActivation
├── merge.go          # 覆盖/合并策略
└── validate.go       # schema/引用校验
```
